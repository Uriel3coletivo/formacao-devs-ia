    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-lg py-4' : 'bg-transparent py-6'}`}>
      <div className="container-custom relative">
        <div className="flex justify-between items-center">
          ...
          {/* Agrupamento: Links + Botão (Tudo na Direita) */}
          <div className="hidden xl:flex items-center gap-10">
            ...
            <button className="bg-[#77df40] text-[#000f44] px-8 py-3 rounded-full font-bold text-sm hover:bg-[#0064f5] hover:text-white transition-all transform hover:scale-105 active:scale-95">
              Inscreva-se
            </button>
          </div>
        ...
        <div className={`xl:hidden absolute top-full left-0 w-full bg-white shadow-xl transition-all duration-300 overflow-hidden flex flex-col items-center ${menuOpen ? 'max-h-[400px] py-6' : 'max-h-0 py-0'}`}>
          {menuItems.map((item) => (
            <button key={item.id} onClick={() => scrollTo(item.id)} className="text-[#000f44] hover:text-[#0064f5] font-medium w-full text-center py-3">
              {item.label}
            </button>
          ))}
          <button className="bg-[#77df40] text-[#000f44] px-8 py-3 rounded-full font-bold text-sm mt-4">
            Inscreva-se
          </button>
        </div>
      </div>
